...
contract People {
	...
	function getPeopleCount() public constant returns(uint) {                
        return people.length;
    }

	function addPerson(bytes32 _firstName, bytes32 _lastName, uint _age) returns (bool success) {

		Person memory newPerson; //doesn't cost gas
		newPerson.firstName = _firstName;
		newPerson.lastName = _lastName;
		newPerson.age = _age;

		people.push(newPerson); //adding element to the array people from line 5; this changes state
		return true;
	}
